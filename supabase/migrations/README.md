...
- **Secrets:** `member_credentials.passcode_hash` is bcrypt (pgcrypto). `api_keys` stores only the SHA-256 hash + prefix (`bk_live_…`); the raw key is shown once at creation and never persisted. `webhooks.secret` and `integrations.config` hold sensitive values: members can set them (insert/update, for creation and rotation) but are never granted SELECT on them — the raw values are only read server-side (e.g. the webhook-dispatcher Edge Function via service_role) for HMAC signing.
